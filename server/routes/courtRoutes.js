...
router.post('/cases', requireAuth, requireRole('judge', 'admin'), async (req, res) => {
  const {
    caseTitle, caseType, judgeId, judgeName,
    plaintiffClientId, plaintiffClientName, plaintiffLawyerLicense, plaintiffLawyerName,
    judgeRatingPlaintiff, clientRatingPlaintiff,
    defendantClientId, defendantClientName, defendantLawyerLicense, defendantLawyerName,
    judgeRatingDefendant, clientRatingDefendant, verdict
  } = req.body;

  if (!plaintiffLawyerLicense && !defendantLawyerLicense) {
    return res.status(400).json({ error: 'At least one lawyer license ID is required' });
  }
  ...
  try {
    ...
    res.status(201).json({ message: 'Case registered. Real-time ratings updated.', case: newCase });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
