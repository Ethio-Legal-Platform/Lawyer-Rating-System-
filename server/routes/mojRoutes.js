...
router.post("/verify-license", async (req, res) => {
  ...
  if (!licenseNumber)
    return res
      .status(400)
      .json({ verified: false, error: "License number is required" });
  ...
  if (!record)
    return res
      ...
      .json({
        verified: false,
        error: "License number not found in official MoJ database",
      });
  if (record.status !== "ACTIVE")
    return res
      ...
      .json({
        verified: false,
        error: "License status is inactive or suspended",
      });
  if (
    name &&
    record.fullName.toLowerCase().trim() !== name.toLowerCase().trim()
  ) {
    return res
      .status(400)
      .json({
        verified: false,
        error: `License validation failed. Name on license is "${record.fullName}", but provided "${name}".`,
      });
  }
